...
- July 6, 2025: RESOLVED critical message isolation bug: messages were appearing in wrong chats due to committee-only filtering instead of threadId-based isolation, implemented comprehensive threadId system for proper conversation isolation, migrated all existing messages to use threadId (11 messages across 7 conversation threads), updated all frontend components to use threadId-based messaging, added automatic migration for orphaned messages, messaging system now properly isolates conversations preventing cross-chat contamination
