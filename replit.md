...
- June 30, 2025: Fixed meeting minutes file serving system: resolved "File not found on disk" error by creating new authenticated endpoint `/api/meeting-minutes/:id/file` that properly handles both absolute and relative file paths, corrected file path resolution logic to work with database-stored paths, removed flawed fallback logic that was showing same file for all meetings, each meeting now properly references its own unique document file
