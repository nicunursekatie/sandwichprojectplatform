...
import { Users, Plus, Edit, Trash2, Phone, Mail, User, AlertCircle, MapPin, Star, Building2, UserPlus } from "lucide-react";
...
export default function HostsManagementConsolidated() {
  ...
  const createHostMutation = useMutation({
    mutationFn: async (data: InsertHost) => {
      return await apiRequest('/api/hosts', 'POST', data);
    },
    onSuccess: () => {
  ...
  const updateHostMutation = useMutation({
    mutationFn: async (data: { id: number; updates: Partial<Host> }) => {
      return await apiRequest(`/api/hosts/${data.id}`, 'PATCH', data.updates);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/hosts-with-contacts'] });
      setEditingHost(null);
  ...
  const deleteHostMutation = useMutation({
    mutationFn: async (id: number) => {
      return await apiRequest(`/api/hosts/${id}`, 'DELETE');
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/hosts-with-contacts'] });
      toast({
  ...
  const createContactMutation = useMutation({
    mutationFn: async (data: InsertHostContact) => {
      return await apiRequest('/api/host-contacts', 'POST', data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/hosts-with-contacts'] });
    ...
    <div className="space-y-6">
      ...
      {hosts.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600">Add your first host to get started with collection management.</p>
        </div>
      )}
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center">
              ...
              {selectedHost?.name} - Contact Management
            </DialogTitle>
          </DialogHeader>
            ...
            <Tabs defaultValue="contacts" className="w-full">
              ...
              <TabsContent value="contacts" className="space-y-4">
                <div className="flex justify-between items-center">
                  <h4 className="text-lg font-medium">Contact List</h4>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex space-x-2 ml-4">
                          ...
                          <Button 
                            ...
                            disabled={deleteContactMutation.isPending || !canEdit}
                          >
